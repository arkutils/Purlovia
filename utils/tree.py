...
class Node(Generic[T]):
    ...
    def translate(self, fn: Callable[[T], Optional[Tdest]], parent: Optional[Node[Tdest]] = None) -> Optional[Node[Tdest]]:
        '''
        ...
        '''
        new_root = fn(self.data)
        if new_root is None:
            return None
        out = Node[Tdest](new_root, parent)
        for node in self.nodes:
            translated_node = node.translate(fn, out)
            if translated_node is not None:
                out.add(translated_node)
        return out
