...
def get_cached_data(key, data_fn, mocker: MockFixture, name='data', force=False):
    data_fn_mock = mocker.Mock(wraps=data_fn, name='data_fn')
...
def test_cached_data_generates_and_stores_data(mocker: MockFixture):
    key = dict(version=1)
    data_fn = lambda key: str(key) * 10
    expected_result = data_fn(key)

    result, calls = get_cached_data(key, data_fn, mocker)
...
def test_cached_data_cache_invalided_on_key_change(mocker: MockFixture):
    ...
    key['version'] = 2
    expected_new_result = data_fn(key)
...
def test_cached_data_allow_force_regerenate(mocker: MockFixture):
    ...
    result, calls = get_cached_data(key, data_fn, mocker, force=True)
    assert result == expected_result
    assert calls == 1
