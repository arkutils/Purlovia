...
class ArrayElement(Element):
    ...
    def decode(self, format, s):
        ...
        # ie 2*'4s' != '24s'
        return [
            self.basic_element.decode(format, x)
            for x in struct.unpack('%s%s' % (format, self.num * self.basic_element.typecode), s)
        ]

    def encode(self, format, vals):
        fmt = format + (self.basic_element.typecode * self.num)
        return struct.pack(fmt, *[self.basic_element.encode(format, v) for v in vals])
...
class MetaStruct(type):
    ...
    def __init__(cls, name, bases, d):
        ...
        elems = sorted(((k, v) for (k, v) in d.items() if isinstance(v, Element)), key=lambda x: x[1].id)
...
class Struct(object, metaclass=MetaStruct):
    ...
    def _pack(self):
        return struct.pack(self._format + self._struct_data,
                           *[elem.encode(self._format, getattr(self, name)) for (name, elem) in self._struct_info])

    def __str__(self):
        pairs = [(k, getattr(self, k)) for k, elem in self._struct_info]
        return "%s(%s)" % (self.__class__.__name__, ', '.join(f'{k}={v}' for k, v in pairs))
