...
class Species(ExportModel):
    ...
    cloning: Optional[CloningData] = Field(
        None,
        description=
        "Full cost is determined by Ceil(costBase + costLevel x CharacterLevel). Cloning time is determined by (timeBase + timeLevel x CharacterLevel) / BabyMatureSpeedMulti."
    )
